...
for _ in range(1000):
    ...
    plt.plot(np.linspace(np.min(x), np.max(x), 100).reshape((-1, 1)), pred)
    ...
    xx = np.hstack((np.ones((x_batch.shape[0], 1)), x_batch))
    theta = np.dot(np.linalg.inv(np.dot(xx.T, xx)), np.dot(xx.T, y_batch))
